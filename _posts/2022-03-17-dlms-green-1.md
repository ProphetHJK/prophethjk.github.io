...
- Frame format field:
  - Format type sub-field (4 bit):3，表示type3(HDLC frame format type 3)
  - the Segmentation bit (S, 1 bit):
  - the frame length sub-field (11 bit):除了flag外的长度
- Destination and source address fields
- Control field：控制字段，见8.4.3
- Header check sequence (HCS) field
- Information field：携带信息域（比如MSDU）,I and UI帧可用(其他类型帧可能也有，后面提到了Disconnect (DISC) command)
- Frame check sequence (FCS) field：计算校验，除flag和FCS
...
> 每个字节的LSB用于标识是否有后续字节，所以`不计入`实际字节的表示，也就是一个字节就`7个有效位`。1111 1111表示一个字节，后续无字节，然后把LSB去掉就是1111 111，前面补0就是0111 1111，一个字节最大就能表示`0x7F`。如果要表示`0xFF`,就是，0000 0010 1111 1111，去掉无效位（每个字节的LSB），0000 001 1111 111，合并填0，就是0000 0000 1111 1111。对于4个字节的情况，分高地址2字节和低地址2字节考虑。
...
- Frame reject (FRMR) response
  ...
  `报告接收到的帧的错误`，该错误`不能通过重传`解决，且该错误不是由于FCS错误引起的：
...
- 8.4.5.4.8 Handling the CALLING device physical address
  ...
  多点网络中，所有服务端收到后首先判断自身的`CALLING DEVICE`标志，如为`TRUE`，表示自己正在寻求发起连接，则接收，如果为FALSE，则丢弃。（收到PH-ABORT.indication 物理层断开后应该置为FALSE）
...
TODO:这里又提到了Partial service invocations，就是用FIRST-PART之类的分包，属于可选的Additional_Service_Parameters参数，与GET之类的原语的service-specific block transfer不一样，和general block transfer（GBT）又不一样，那这三种分包方式可以同时存在吗
...
- streaming(`STR`) :对于一个窗口，在过程中(STR = TRUE(1))还是已结束(STR = FALSE(0)，一个窗口内的`最后一包`)。如果是已结束，需要`对方`回个`确认`，这个确认是对这个窗口的，如果是完整APDU的最后一包则`不回`(`最后一个窗口不确认`)（TODO:怎么判断收全了）
...
- **9.4.6.13.4 Send GBT APDU stream**
  ...
  - Confirmed GBT stream send
    ...
    被确认后才能从SQ删除，最后一包不需要对方确认，所以不能直接从SQ中删除。TODO:应该什么时候删除，文中也没说
  - Unconfirmed GBT send

    AP调用原语参数`BTW`要等于0

    发完就清SQ，不用确认
- **9.4.6.13.5 Process GBT APDU**
...
Send Queue SQ：发送队列，对于block
  ...
  对于SQ内blocks的发送是个流过程（TODO:意思是不是已经和调用原语无关了）

Receive Queue RQ：接收队列，对于block
...
TODO:如果4、5两包都丢了，是不是就是先补4，再补5，不会两包一起补。更新：如果4、5丢了，就W置2，BNA置3，可以表示丢了连续的两个包

如果是最后一包LB=1的丢了，由客户端请求这一包

![messages](/assets/img/2022-03-17-dlms-green-1/setgbt.jpg)

TODO:为什么Action-Request-With-List收到一半，服务端就可以回Action-Response-With-List
