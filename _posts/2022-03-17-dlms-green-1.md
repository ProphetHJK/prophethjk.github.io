...
- [1 Scope](#1-scope)
- [3 Terms, definitions and abbreviations and symbols](#3-terms-definitions-and-abbreviations-and-symbols)
  - [3.1 General DLMS/COSEM definitions](#31-general-dlmscosem-definitions)
- [4 Information exchange in DLMS/COSEM](#4-information-exchange-in-dlmscosem)
  - [4.1 General](#41-general)
  - [4.2 Communication model](#42-communication-model)
  - [4.3 Naming and addressing](#43-naming-and-addressing)
    - [4.3.3 Addressing](#433-addressing)
    - [4.3.4 System title](#434-system-title)
    - [4.3.5 Logical Device Name](#435-logical-device-name)
  - [4.4 Connection oriented operation](#44-connection-oriented-operation)
  - [4.5 Application associations](#45-application-associations)
    - [4.5.2 Application context](#452-application-context)
    - [4.5.3 Authentication](#453-authentication)
    - [4.5.4 xDLMS context](#454-xdlms-context)
    - [4.5.5 Security context](#455-security-context)
    - [4.5.6 Access rights](#456-access-rights)
  - [4.6 Messaging patterns](#46-messaging-patterns)
  - [4.8 Communication profiles](#48-communication-profiles)
  - [4.9 Model of a DLMS/COSEM system](#49-model-of-a-dlmscosem-system)
  - [4.10 Model of DLMS servers](#410-model-of-dlms-servers)
  - [4.11 Model of a DLMS client](#411-model-of-a-dlms-client)
  - [4.12 Interoperability and interconnectivity in DLMS/COSEM](#412-interoperability-and-interconnectivity-in-dlmscosem)
  - [4.13 Ensuring interconnectivity: the protocol identification service](#413-ensuring-interconnectivity-the-protocol-identification-service)
  - [4.14 System integration and installation](#414-system-integration-and-installation)
- [5 Physical layer services and procedures for connection-oriented asynchronous data exchange](#5-physical-layer-services-and-procedures-for-connection-oriented-asynchronous-data-exchange)
  - [5.1 Overview](#51-overview)
  - [5.2 Service specification](#52-service-specification)
    - [5.2.1 List of services](#521-list-of-services)
    - [5.2.2 Use of the physical layer services](#522-use-of-the-physical-layer-services)
    - [5.2.3 Service definitions](#523-service-definitions)
  - [5.3 Protocol specification](#53-protocol-specification)
    - [5.3.1 Physical layer protocol data unit](#531-physical-layer-protocol-data-unit)
    - [5.3.2 Transmission order and characteristics](#532-transmission-order-and-characteristics)
    - [5.3.3 Physical layer operation – description of the procedures](#533-physical-layer-operation--description-of-the-procedures)
      - [5.3.3.1 General](#5331-general)
      - [5.3.3.2 Setting up a physical connection](#5332-setting-up-a-physical-connection)
      - [5.3.3.3 The Identification service](#5333-the-identification-service)
      - [5.3.3.4 Data transfer](#5334-data-transfer)
      - [5.3.3.5 Disconnection of an existing physical connection](#5335-disconnection-of-an-existing-physical-connection)
  - [5.4 example: PhL service primitives and Hayes commands](#54-example-phl-service-primitives-and-hayes-commands)
- [6 Direct Local Connection](#6-direct-local-connection)
- [7 DLMS/COSEM transport layer for IP networks](#7-dlmscosem-transport-layer-for-ip-networks)
  - [7.2 The TCP-UDP/IP based transport layers](#72-the-tcp-udpip-based-transport-layers)
    - [7.2.3 The DLMS/COSEM connection-less, UDP-based transport layer](#723-the-dlmscosem-connection-less-udp-based-transport-layer)
    - [7.2.3.3.2 The wrapper protocol data unit (WPDU)](#72332-the-wrapper-protocol-data-unit-wpdu)
    - [7.2.4 The DLMS/COSEM connection-oriented, TCP-based transport layer](#724-the-dlmscosem-connection-oriented-tcp-based-transport-layer)
      - [TCP-CONNECT](#tcp-connect)
      - [TCP-DISCONNECT](#tcp-disconnect)
      - [TCP-ABORT](#tcp-abort)
      - [TCP-DATA](#tcp-data)
      - [7.2.4.3 Protocol specification for the DLMS/COSEM TCP-based transport layer](#7243-protocol-specification-for-the-dlmscosem-tcp-based-transport-layer)
        - [7.2.4.3.5 Definition of the procedures](#72435-definition-of-the-procedures)
    - [7.2.5 Converting OSI-style TL services to and from RFC-style TCP function calls](#725-converting-osi-style-tl-services-to-and-from-rfc-style-tcp-function-calls)
      - [7.2.5.1 Transport layer and TCP connection establishment](#7251-transport-layer-and-tcp-connection-establishment)
      - [7.2.5.2 Closing a transport layer and a TCP connection](#7252-closing-a-transport-layer-and-a-tcp-connection)
      - [7.2.5.3 TCP connection abort](#7253-tcp-connection-abort)
      - [7.2.5.4 Data transfer using the TCP-DATA service](#7254-data-transfer-using-the-tcp-data-service)
  - [7.3 The DLMS/COSEM CoAP based transport layer](#73-the-dlmscosem-coap-based-transport-layer)
    - [7.3.2 Overview](#732-overview)
    - [7.3.3 Structure of the DLMS/COSEM CoAP transport layer](#733-structure-of-the-dlmscosem-coap-transport-layer)
      - [7.3.3.2 Identification and addressing](#7332-identification-and-addressing)
        - [7.3.3.2.2 DLMS/COSEM AL identification within the CoAP transport layer](#73322-dlmscosem-al-identification-within-the-coap-transport-layer)
        - [DLMS/COSEM CoAP transport layer SAPs](#dlmscosem-coap-transport-layer-saps)
    - [7.3.4 Service specification for the DLMS/COSEM CoAP transport layer](#734-service-specification-for-the-dlmscosem-coap-transport-layer)
      - [7.3.4.2 The DLMS/COSEM CoAP-DATA service primitives](#7342-the-dlmscosem-coap-data-service-primitives)
        - [7.3.4.2.1 CoAP-DATA.request](#73421-coap-datarequest)
        - [7.3.4.2.2 CoAP-DATA.indication](#73422-coap-dataindication)
        - [7.3.4.2.2 CoAP-DATA.confirm](#73422-coap-dataconfirm)
    - [7.3.5 Protocol specification of the DLMS/COSEM CoAP transport layer](#735-protocol-specification-of-the-dlmscosem-coap-transport-layer)
      - [7.3.5.2 The DLMS/COSEM CoAP TL Protocol Data Unit (CoAP-PDU)](#7352-the-dlmscosem-coap-tl-protocol-data-unit-coap-pdu)
      - [7.3.5.3 The DLMS/COSEM CoAP Wrapper Protocol Data Unit (CWPDU)](#7353-the-dlmscosem-coap-wrapper-protocol-data-unit-cwpdu)
      - [7.3.5.4 The Constrained Application Protocol (CoAP)](#7354-the-constrained-application-protocol-coap)
        - [7.3.5.4.2 The CoAP Message](#73542-the-coap-message)
        - [7.3.5.4.3 CoAP retransmission and response piggybacking](#73543-coap-retransmission-and-response-piggybacking)
        - [7.3.5.4.5 CoAP Block Transfer](#73545-coap-block-transfer)
      - [7.3.5.5 Error Handling](#7355-error-handling)
        - [7.3.5.5.2 CoAP protocol layers](#73552-coap-protocol-layers)
        - [7.3.5.5.3 CoAP wrapper layer](#73553-coap-wrapper-layer)
        - [7.3.5.5.4 Propagation of errors through CoAP wrapper layer](#73554-propagation-of-errors-through-coap-wrapper-layer)
      - [7.3.5.6 DLMS/COSEM CoAP TL confirmations](#7356-dlmscosem-coap-tl-confirmations)
      - [7.3.5.7 CoAP wrapper state machine](#7357-coap-wrapper-state-machine)
        - [7.3.5.7.2 CoAP DLMS/COSEM wrapper request/response context](#73572-coap-dlmscosem-wrapper-requestresponse-context)
        - [7.3.5.7.4 CoAP-DATA.request invocation handling](#73574-coap-datarequest-invocation-handling)
        - [7.3.5.7.5 Handling of incoming CWPDU or CoAP layer transmission failures](#73575-handling-of-incoming-cwpdu-or-coap-layer-transmission-failures)
        - [7.3.5.7.6 Garbage collection](#73576-garbage-collection)
    - [7.3.6 DLMS/COSEM CoAP TL Data Transfers](#736-dlmscosem-coap-tl-data-transfers)
      - [7.3.6.2 General transfer of confirmed DLMS/COSEM AL service requests](#7362-general-transfer-of-confirmed-dlmscosem-al-service-requests)
      - [7.3.6.3 Reliable DLMS/COSEM CoAP TL operation](#7363-reliable-dlmscosem-coap-tl-operation)
      - [7.3.6.4 Unreliable DLMS/COSEM CoAP TL operation](#7364-unreliable-dlmscosem-coap-tl-operation)
      - [7.3.6.5 DLMS/COSEM CoAP Block Transfer operation](#7365-dlmscosem-coap-block-transfer-operation)
      - [7.3.6.6 DLMS GBT operation over DLMS/COSEM CoAP TL](#7366-dlms-gbt-operation-over-dlmscosem-coap-tl)
- [8 Data Link Layer using the HDLC protocol](#8-data-link-layer-using-the-hdlc-protocol)
  - [8.1 Overview](#81-overview)
    - [8.1.2 Structure of the data link layer](#812-structure-of-the-data-link-layer)
    - [8.1.3 Specification method](#813-specification-method)
  - [8.2 Service specification](#82-service-specification)
    - [8.2.2 Setting up the data link connection: the DL-CONNECT and MA-CONNECT services](#822-setting-up-the-data-link-connection-the-dl-connect-and-ma-connect-services)
      - [8.2.2.2 DL-CONNECT.request and MA-CONNECT.request](#8222-dl-connectrequest-and-ma-connectrequest)
      - [8.2.2.3 DL-CONNECT.indication and MA-CONNECT.indication](#8223-dl-connectindication-and-ma-connectindication)
      - [8.2.2.4 DL-CONNECT.response and MA-CONNECT.response](#8224-dl-connectresponse-and-ma-connectresponse)
      - [8.2.2.5 DL-CONNECT.confirm and MA-CONNECT.confirm](#8225-dl-connectconfirm-and-ma-connectconfirm)
    - [8.2.3 Disconnecting the data link connection: the DL-DISCONNECT and MA-DISCONNECT services](#823-disconnecting-the-data-link-connection-the-dl-disconnect-and-ma-disconnect-services)
      - [8.2.3.2 DL-DISCONNECT.request and MA-DISCONNECT.request](#8232-dl-disconnectrequest-and-ma-disconnectrequest)
      - [8.2.3.3 DL-DISCONNECT.indication and MA-DISCONNECT.indication](#8233-dl-disconnectindication-and-ma-disconnectindication)
      - [8.2.3.4 DL-DISCONNECT.response and MA-DISCONNECT.response](#8234-dl-disconnectresponse-and-ma-disconnectresponse)
      - [8.2.3.5 DL-DISCONNECT.confirm and MA-DISCONNECT.confirm](#8235-dl-disconnectconfirm-and-ma-disconnectconfirm)
    - [8.2.4 Data transfer: the DL-DATA and MA-DATA services](#824-data-transfer-the-dl-data-and-ma-data-services)
      - [8.2.4.2 DL-DATA.request and MA-DATA.request](#8242-dl-datarequest-and-ma-datarequest)
      - [8.2.4.3 DL-DATA.indication and MA-DATA.indication](#8243-dl-dataindication-and-ma-dataindication)
      - [8.2.4.4 DL-DATA.confirm and MA-DATA.confirm](#8244-dl-dataconfirm-and-ma-dataconfirm)
    - [8.2.5 Physical layer services used by the MAC sublayer](#825-physical-layer-services-used-by-the-mac-sublayer)
      - [8.2.5.4 Data transfer](#8254-data-transfer)
  - [8.3 Protocol specification for the LLC sublayer](#83-protocol-specification-for-the-llc-sublayer)
    - [8.3.2 LLC PDU format](#832-llc-pdu-format)
    - [8.3.3 State transition tables for the LLC sublayer](#833-state-transition-tables-for-the-llc-sublayer)
  - [8.4 Protocol specification for the MAC sublayer](#84-protocol-specification-for-the-mac-sublayer)
    - [8.4.1 The MAC PDU and the HDLC frame](#841-the-mac-pdu-and-the-hdlc-frame)
      - [8.4.1.1 HDLC frame format type 3](#8411-hdlc-frame-format-type-3)
    - [8.4.2 MAC addressing](#842-mac-addressing)
      - [8.4.2.2 Address field structure](#8422-address-field-structure)
      - [8.4.2.3 Reserved special HDLC addresses](#8423-reserved-special-hdlc-addresses)
      - [8.4.2.4 Handling special addresses](#8424-handling-special-addresses)
      - [8.4.2.5 Handling inopportune address lengths in the server](#8425-handling-inopportune-address-lengths-in-the-server)
    - [8.4.3 Command and response frames](#843-command-and-response-frames)
    - [8.4.4 Elements of the procedures](#844-elements-of-the-procedures)
      - [8.4.4.2 Transmission considerations](#8442-transmission-considerations)
      - [8.4.4.3 HDLC channel states](#8443-hdlc-channel-states)
    - [8.4.5 HDLC channel operation – Description of the procedures](#845-hdlc-channel-operation--description-of-the-procedures)
      - [8.4.5.2 Data station characteristics](#8452-data-station-characteristics)
      - [8.4.5.3 Procedures for setting up and disconnecting the data link](#8453-procedures-for-setting-up-and-disconnecting-the-data-link)
      - [8.4.5.4 Procedures for data exchange](#8454-procedures-for-data-exchange)
      - [8.4.5.5 Exception recovery](#8455-exception-recovery)
      - [8.4.5.6 Time-outs and other MAC sublayer parameters](#8456-time-outs-and-other-mac-sublayer-parameters)
      - [8.4.5.7 State transition diagram for the server MAC sublayer](#8457-state-transition-diagram-for-the-server-mac-sublayer)
  - [8.5 FCS calculation](#85-fcs-calculation)
    - [8.5.1 Test sequence for the FCS calculation](#851-test-sequence-for-the-fcs-calculation)
    - [8.5.2 Fast frame check sequence (FCS) implementation](#852-fast-frame-check-sequence-fcs-implementation)
    - [8.5.3 16-bit FCS computation method](#853-16-bit-fcs-computation-method)
  - [8.6 Data link layer management services](#86-data-link-layer-management-services)
- [9 DLMS/COSEM application layer](#9-dlmscosem-application-layer)
  - [9.1 DLMS/COSEM application layer main features](#91-dlmscosem-application-layer-main-features)
    - [9.1.2 DLMS/COSEM application layer structure](#912-dlmscosem-application-layer-structure)
    - [9.1.3 The Association Control Service Element, ACSE](#913-the-association-control-service-element-acse)
    - [9.1.4 The xDLMS application service element](#914-the-xdlms-application-service-element)
      - [9.1.4.2 The xDLMS initiate service](#9142-the-xdlms-initiate-service)
      - [9.1.4.3 COSEM object related xDLMS services](#9143-cosem-object-related-xdlms-services)
        - [9.1.4.3.2 xDLMS services used by the client with LN referencing](#91432-xdlms-services-used-by-the-client-with-ln-referencing)
        - [9.1.4.3.3 xDLMS services used by the client with SN referencing](#91433-xdlms-services-used-by-the-client-with-sn-referencing)
      - [9.1.4.3.4 Unsolicited services](#91434-unsolicited-services)
        - [9.1.4.3.5 Selective access](#91435-selective-access)
        - [9.1.4.3.6 Multiple references](#91436-multiple-references)
        - [9.1.4.3.7 Attribute_0 referencing](#91437-attribute_0-referencing)
      - [9.1.4.4 Additional mechanisms](#9144-additional-mechanisms)
        - [9.1.4.4.2 Referencing methods and service mapping](#91442-referencing-methods-and-service-mapping)
        - [9.1.4.4.3 Identification of service invocations: the Invoke_Id parameter](#91443-identification-of-service-invocations-the-invoke_id-parameter)
        - [9.1.4.4.4 Priority handling](#91444-priority-handling)
        - [9.1.4.4.5 Transferring long messages](#91445-transferring-long-messages)
        - [9.1.4.4.6 Composable xDLMS messages](#91446-composable-xdlms-messages)
        - [9.1.4.4.7 Compression and decompression](#91447-compression-and-decompression)
        - [9.1.4.4.8 General protection](#91448-general-protection)
        - [General block transfer (GBT)](#general-block-transfer-gbt)
      - [9.1.4.5 Additional data types](#9145-additional-data-types)
      - [9.1.4.6 xDLMS version number](#9146-xdlms-version-number)
      - [9.1.4.7 xDLMS conformance block](#9147-xdlms-conformance-block)
      - [9.1.4.8 Maximum PDU size](#9148-maximum-pdu-size)
    - [9.1.5 Layer management services](#915-layer-management-services)
    - [9.1.6 Summary of DLMS/COSEM application layer services](#916-summary-of-dlmscosem-application-layer-services)
    - [9.1.7 DLMS/COSEM application layer protocols](#917-dlmscosem-application-layer-protocols)
  - [9.2 Information security in DLMS/COSEM](#92-information-security-in-dlmscosem)
    - [9.2.2 The DLMS/COSEM security concept](#922-the-dlmscosem-security-concept)
      - [9.2.2.2 Identification and authentication](#9222-identification-and-authentication)
        - [9.2.2.2.1 Identification](#92221-identification)
        - [9.2.2.2.2 Authentication mechanisms](#92222-authentication-mechanisms)
      - [9.2.2.3 Security context](#9223-security-context)
      - [9.2.2.4 Access rights](#9224-access-rights)
      - [9.2.2.5 Application layer message security](#9225-application-layer-message-security)
      - [9.2.2.6 COSEM data security](#9226-cosem-data-security)
    - [9.2.3 Cryptographic algorithms](#923-cryptographic-algorithms)
      - [9.2.3.2 Hash function](#9232-hash-function)
      - [9.2.3.3 Symmetric key algorithms](#9233-symmetric-key-algorithms)
        - [9.2.3.3.2 Encryption and decryption](#92332-encryption-and-decryption)
        - [9.2.3.3.3 Advanced Encryption Standard](#92333-advanced-encryption-standard)
        - [9.2.3.3.4 Encryption Modes of Operation](#92334-encryption-modes-of-operation)
      - [9.2.3.3.5 Message Authentication Code](#92335-message-authentication-code)
      - [9.2.3.3.6 Key wrapping](#92336-key-wrapping)
        - [9.2.3.3.7 Galois/Counter Mode](#92337-galoiscounter-mode)
        - [9.2.3.3.8 AES key wrap](#92338-aes-key-wrap)
      - [9.2.3.4 Public key algorithms](#9234-public-key-algorithms)
        - [9.2.3.4.2 Elliptic curve cryptography](#92342-elliptic-curve-cryptography)
        - [9.2.3.4.3 Data conversions](#92343-data-conversions)
        - [9.2.3.4.4 Digital signature](#92344-digital-signature)
        - [9.2.3.4.5 Elliptic curve digital signature (ECDSA)](#92345-elliptic-curve-digital-signature-ecdsa)
        - [9.2.3.4.6 Key agreement](#92346-key-agreement)
      - [9.2.3.5 Random number generation](#9235-random-number-generation)
      - [9.2.3.6 Compression](#9236-compression)
      - [9.2.3.7 Security suite](#9237-security-suite)
    - [9.2.4 Cryptographic keys – overview](#924-cryptographic-keys--overview)
    - [9.2.5 Key used with symmetric key algorithms](#925-key-used-with-symmetric-key-algorithms)
      - [9.2.5.1 Symmetric keys types](#9251-symmetric-keys-types)
      - [9.2.5.2 Key information with general-ciphering APDU and data protection](#9252-key-information-with-general-ciphering-apdu-and-data-protection)
      - [9.2.5.3 Key identification](#9253-key-identification)
      - [9.2.5.4 Key wrapping](#9254-key-wrapping)
      - [9.2.5.5 Key agreement](#9255-key-agreement)
      - [9.2.5.6 Symmetric key cryptoperiods](#9256-symmetric-key-cryptoperiods)
    - [9.2.6 Keys used with public key algorithms](#926-keys-used-with-public-key-algorithms)
      - [9.2.6.2 Key pair generation](#9262-key-pair-generation)
      - [9.2.6.3 Public key certificates and infrastructure](#9263-public-key-certificates-and-infrastructure)
        - [9.2.6.3.2 Trust model](#92632-trust-model)
        - [9.2.6.3.3 PKI architecture – informative](#92633-pki-architecture--informative)
      - [9.2.6.4 Certificate and certificate extension profile](#9264-certificate-and-certificate-extension-profile)
        - [9.2.6.4.2 The X.509 v3 Certificate](#92642-the-x509-v3-certificate)
      - [9.2.6.5 Suite B end entity certificate types to be supported by DLMS servers](#9265-suite-b-end-entity-certificate-types-to-be-supported-by-dlms-servers)
      - [9.2.6.6 Management of certificates](#9266-management-of-certificates)
        - [9.2.6.6.2 Provisioning servers with trust anchors](#92662-provisioning-servers-with-trust-anchors)
        - [9.2.6.6.3 Provisioning the server with further CA certificates](#92663-provisioning-the-server-with-further-ca-certificates)
        - [9.2.6.6.4 Security personalisation of the server](#92664-security-personalisation-of-the-server)
        - [9.2.6.6.5 Provisioning servers with certificates of clients and third parties](#92665-provisioning-servers-with-certificates-of-clients-and-third-parties)
        - [9.2.6.6.6 Provisioning clients and third parties with certificates of servers](#92666-provisioning-clients-and-third-parties-with-certificates-of-servers)
        - [9.2.6.6.7 Certificate removal from the server](#92667-certificate-removal-from-the-server)
    - [9.2.7 Applying cryptographic protection](#927-applying-cryptographic-protection)
      - [9.2.7.2 Protecting xDLMS APDUs](#9272-protecting-xdlms-apdus)
        - [9.2.7.2.2 Security policy and access rights values](#92722-security-policy-and-access-rights-values)
        - [9.2.7.2.3 Ciphered xDLMS APDUs](#92723-ciphered-xdlms-apdus)
        - [9.2.7.2.4 Encryption, authentication and compression](#92724-encryption-authentication-and-compression)
        - [9.2.7.2.5 Digital signature](#92725-digital-signature)
      - [9.2.7.3 Multi-layer protection by multiple parties](#9273-multi-layer-protection-by-multiple-parties)
      - [9.2.7.4 HLS authentication mechanisms](#9274-hls-authentication-mechanisms)
      - [9.2.7.5 Protecting COSEM data](#9275-protecting-cosem-data)
  - [9.3 DLMS/COSEM application layer service specification](#93-dlmscosem-application-layer-service-specification)
    - [9.3.1 Service primitives and parameters](#931-service-primitives-and-parameters)
    - [9.3.2 The COSEM-OPEN service](#932-the-cosem-open-service)
      - [9.3.2.3 Use](#9323-use)
    - [9.3.3 The COSEM-RELEASE service](#933-the-cosem-release-service)
    - [9.3.4 The COSEM-ABORT service](#934-the-cosem-abort-service)
    - [9.3.5 Protection and general block transfer parameters](#935-protection-and-general-block-transfer-parameters)
    - [9.3.6 The GET service](#936-the-get-service)
    - [9.3.7 The SET service](#937-the-set-service)
    - [9.3.8 The ACTION service](#938-the-action-service)
    - [9.3.9 The ACCESS service](#939-the-access-service)
      - [9.3.9.2 Service specification](#9392-service-specification)
    - [9.3.10 The DataNotification service](#9310-the-datanotification-service)
    - [9.3.11 The EventNotification service](#9311-the-eventnotification-service)
    - [9.3.12 The TriggerEventNotificationSending service](#9312-the-triggereventnotificationsending-service)
    - [9.3.13 Variable access specification](#9313-variable-access-specification)
    - [9.3.14 The Read service](#9314-the-read-service)
    - [9.3.15 The Write service](#9315-the-write-service)
    - [9.3.16 The UnconfirmedWrite service](#9316-the-unconfirmedwrite-service)
    - [9.3.17 The InformationReport service](#9317-the-informationreport-service)
    - [9.3.18 Client side layer management services: the SetMapperTable.request](#9318-client-side-layer-management-services-the-setmappertablerequest)
  - [9.4 DLMS/COSEM application layer protocol specification](#94-dlmscosem-application-layer-protocol-specification)
    - [9.4.1 The control function (CF)](#941-the-control-function-cf)
      - [9.4.1.1 State definitions of the client side control function](#9411-state-definitions-of-the-client-side-control-function)
      - [9.4.1.2 State definitions of the server side control function](#9412-state-definitions-of-the-server-side-control-function)
    - [9.4.2 The ACSE services and APDUs](#942-the-acse-services-and-apdus)
      - [9.4.2.1 ACSE functional units, services and service parameters](#9421-acse-functional-units-services-and-service-parameters)
      - [9.4.2.2 Registered COSEM names](#9422-registered-cosem-names)
    - [9.4.3 APDU encoding rules](#943-apdu-encoding-rules)
      - [9.4.3.1 Encoding of the ACSE APDUs](#9431-encoding-of-the-acse-apdus)
      - [9.4.3.2 Encoding of the xDLMS APDUs](#9432-encoding-of-the-xdlms-apdus)
      - [9.4.3.3 XML](#9433-xml)
    - [9.4.4 Protocol for application association establishment](#944-protocol-for-application-association-establishment)
      - [9.4.4.1 Protocol for the establishment of confirmed application associations](#9441-protocol-for-the-establishment-of-confirmed-application-associations)
      - [9.4.4.2 Repeated COSEM-OPEN service invocations](#9442-repeated-cosem-open-service-invocations)
      - [9.4.4.3 Establishment of unconfirmed application associations](#9443-establishment-of-unconfirmed-application-associations)
      - [9.4.4.4 Pre-established application associations](#9444-pre-established-application-associations)
    - [9.4.5 Protocol for application association release](#945-protocol-for-application-association-release)
    - [9.4.6 Protocol for the data transfer services](#946-protocol-for-the-data-transfer-services)
      - [9.4.6.1 Negotiation of services and options – the conformance block](#9461-negotiation-of-services-and-options--the-conformance-block)
      - [9.4.6.2 Confirmed and unconfirmed xDLMS service invocations](#9462-confirmed-and-unconfirmed-xdlms-service-invocations)
      - [9.4.6.3 Protocol for the GET service](#9463-protocol-for-the-get-service)
      - [9.4.6.4 Protocol for the SET service](#9464-protocol-for-the-set-service)
      - [9.4.6.5 Protocol for the ACTION service](#9465-protocol-for-the-action-service)
      - [9.4.6.6 Protocol for the ACCESS service](#9466-protocol-for-the-access-service)
      - [9.4.6.7 Protocol of the DataNotification service](#9467-protocol-of-the-datanotification-service)
      - [9.4.6.8 Protocol for the EventNotification service](#9468-protocol-for-the-eventnotification-service)
      - [9.4.6.9 Protocol for the Read service](#9469-protocol-for-the-read-service)
      - [9.4.6.10 Protocol for the Write service](#94610-protocol-for-the-write-service)
      - [9.4.6.11 Protocol for the UnconfirmedWrite service](#94611-protocol-for-the-unconfirmedwrite-service)
      - [9.4.6.12 Protocol for the InformationReport service](#94612-protocol-for-the-informationreport-service)
      - [9.4.6.13 Protocol of general block transfer mechanism](#94613-protocol-of-general-block-transfer-mechanism)
        - [9.4.6.13.2 The GBT procedure](#946132-the-gbt-procedure)
        - [9.4.6.13.3 GBT procedure state variables](#946133-gbt-procedure-state-variables)
        - [9.4.6.13.7 GBT protocol examples](#946137-gbt-protocol-examples)
        - [9.4.6.13.8 Aborting the GBT procedure](#946138-aborting-the-gbt-procedure)
      - [9.4.6.14 Protocol of exception mechanism](#94614-protocol-of-exception-mechanism)
  - [9.5 Abstract syntax of COSEM PDUs](#95-abstract-syntax-of-cosem-pdus)
  - [9.6 COSEM PDU XML schema](#96-cosem-pdu-xml-schema)
- [10 Using the DLMS/COSEM application layer in various communications profiles](#10-using-the-dlmscosem-application-layer-in-various-communications-profiles)
  - [10.1 Communication profile specific elements](#101-communication-profile-specific-elements)
  - [10.2 The 3-layer, connection-oriented, HDLC based ommunication profile](#102-the-3-layer-connection-oriented-hdlc-based-ommunication-profile)
    - [10.2.2 The structure of the profile](#1022-the-structure-of-the-profile)
    - [10.2.3 Identification and addressing scheme](#1023-identification-and-addressing-scheme)
    - [10.2.4 Supporting protocol layer services and service mapping](#1024-supporting-protocol-layer-services-and-service-mapping)
    - [10.2.5 Communication profile specific service parameters of the DLMS/COSEM AL services](#1025-communication-profile-specific-service-parameters-of-the-dlmscosem-al-services)
    - [10.2.6 Specific considerations / constraints](#1026-specific-considerations--constraints)
      - [10.2.6.1 Confirmed and unconfirmed AAs and data transfer service invocations, frame types used](#10261-confirmed-and-unconfirmed-aas-and-data-transfer-service-invocations-frame-types-used)
      - [10.2.6.2 Correspondence between AAs and data link layer connections, releasing AAs](#10262-correspondence-between-aas-and-data-link-layer-connections-releasing-aas)
      - [10.2.6.3 Service parameters of the COSEM-OPEN / -RELEASE / -ABORT services](#10263-service-parameters-of-the-cosem-open---release---abort-services)
      - [10.2.6.4 EventNotification service and protocol](#10264-eventnotification-service-and-protocol)
      - [10.2.6.5 Transporting long messages](#10265-transporting-long-messages)
      - [10.2.6.6 Supporting multi-drop configurations](#10266-supporting-multi-drop-configurations)
  - [10.3 The TCP-UDP/IP based communication profiles (COSEM_on_IP)](#103-the-tcp-udpip-based-communication-profiles-cosem_on_ip)
    - [10.3.6 Specific considerations / constraints](#1036-specific-considerations--constraints)
      - [10.3.6.6 Transporting long messages](#10366-transporting-long-messages)
      - [10.3.6.7 Allowing COSEM servers to establish the TCP connection](#10367-allowing-cosem-servers-to-establish-the-tcp-connection)
  - [10.4 The CoAP based communication profile (DLMS/COSEM_on_CoAP)](#104-the-coap-based-communication-profile-dlmscosem_on_coap)
    - [10.4.3 Identification and addressing](#1043-identification-and-addressing)
    - [10.4.4 Supporting layer services and service mapping](#1044-supporting-layer-services-and-service-mapping)
    - [10.4.6 Specific considerations / constraints](#1046-specific-considerations--constraints)
  - [10.8 LPWAN profile](#108-lpwan-profile)
  - [10.9 Wi-SUN profile](#109-wi-sun-profile)
  - [10.10 Gateway protocol](#1010-gateway-protocol)
- [11 AARQ and AARE encoding examples](#11-aarq-and-aare-encoding-examples)
- [12 Encoding examples: AARQ and AARE APDUs using a ciphered application context](#12-encoding-examples-aarq-and-aare-apdus-using-a-ciphered-application-context)
- [13 S-FSK PLC encoding examples](#13-s-fsk-plc-encoding-examples)
- [14 Data transfer service examples](#14-data-transfer-service-examples)
  - [14.4 Profile generic IC buffer attribute encoding examples](#144-profile-generic-ic-buffer-attribute-encoding-examples)
    - [14.4.3 Get-response with Profile generic null-data compressed encoding example](#1443-get-response-with-profile-generic-null-data-compressed-encoding-example)
    - [14.4.4 Get-response with Profile generic compact-array encoding example](#1444-get-response-with-profile-generic-compact-array-encoding-example)
    - [14.4.5 Get-response with Profile generic null-data and delta-value encoding example](#1445-get-response-with-profile-generic-null-data-and-delta-value-encoding-example)
...
一个 COSEM 逻辑设备可以支持一个或多个 AAs，每个 AAs 有一个`不同的客户端`。每个 AA 决定发生信息交换的上下文。
...
在`识别阶段`AP 是可以通过 PH-DATA 原语向物理层传数据的，进`数据传输阶`段就不行了
...
MAC 子层的主要功能包括数据`帧的封装/卸装`，`帧的寻址和识别`，帧的`接收与发送`，`链路的管理`，帧的`差错控制`等。MAC 子层的存在屏蔽了不同物理链路种类的差异性;非常重要的一项功能是仲裁`介质的使用权`，即规定站点何时可以使用通信介质。实际上，局域网技术中是采用具有冲突检测的`载波侦听多路访问`（Carrier Sense Multiple Access /
...
- Frame format field:
  ...
  - the Segmentation bit (S, 1 bit):`类型细分`，对上面的 format type 的继续细分
  - the frame length sub-field (11 bit):除了 flag 外的`长度`
- Destination and source address fields:见 8.4.2
- Control field：`控制字段`，见 8.4.3
- Header check sequence (HCS) field：对头部序列的`校验`，计算`opening flag和HCS之间`的部分，如 Frame format+Dest. address+Src. address+Control，不包括后面的信息域和 FCS。当信息域不存在或者为空时，则 HCS 不存在，仅有 FCS
- Information field：携带`信息域`（比如 MSDU）,I and UI 帧可用(其他类型帧可能也有，后面提到了 Disconnect (DISC) command)
- Frame check sequence (FCS) field：计算`校验`，除 flag 和 FCS
...
P 表示`poll bit`，表示是否交出`发送权`，一般需要响应的请求的最后一帧会置为`TRUE`
...
- Receive not ready (`RNR`) command and response
  ...
  RNR 接收方应该要知道，N(R)-1 之后的 I 帧发送都是无效的，也不应该再发 I 帧了
...
- 扩展 DLMS 应用服务元素 the extended DLMS Application Service Elemen，`xDLMS ASE`;
...
- COSEM-OPEN;
  ...
  COSEM-OPEN 服务用于`建立AAs`。它基于 ACSE A-ASSOCIATE 服务。基于 ASE 过程中的 Application_Context_Name, Security_Mechanism_Name and xDLMS context 参数

  - `confirmed AAs`使用 COSEM-OPEN 服务，可以在单个客户端和单个服务器之间建立；

  - `unconfirmed AAs`使用 COSEM-OPEN 服务，可以在单个客户端和多个服务器见建立，只有客户端发送，服务端不回应。（多播，广播）

  - `pre-established AAs` 可能预先存在。 不使用 COSEM-OPEN 服务。 客户端必须知道服务器支持的上下文。 预先建立的 AA 可以是确认或未确认。
...
- unsolicited: 由服务器端发起，无需请求
  ...
  来自服务器的`未经请求`的`DataNotification`也可能是(见 9.3.10):

  - confirmed:在这种情况下，客户端提供一个`响应`来确认收到了未经请求的 DataNotification
  - unconfirmed:在这种情况下，客户端没有对未经请求的 DataNotification 提供响应

`附加服务`-不是基于 IEC 61334-4-41:1996 规定的 DLMS 服务-是:
...
共有 DataNotification，LN 有 EventNotification，SN 有 InformationReport
...
在 COSEM 对象相关的服务调用中，可以`引用`一个或`多个`命名变量、属性和/或方法，见 9.4.6.1的`multiple-references`参数，比如ACCESS总是支持这个特性
...
如果最大接收 PDU 大小很大，超过了下一层的限制，APDUs 符合条件，不用分块，那就需要下一层进行分包操作

特定于服务的块传输机制用于：

- 使用 LN 参引用的 confirmed services:GET、SET、ACTION;
- 使用 SN 参引用的 confirmed services:Read、Write
...
`特定于服务的块传输`只能一包一包顺序传，不支持流式传输，不支持恢复丢失块。加密是加一个 block，而不是整个 APDU，服务专用数字签名不可用。TODO：服务专用数字签名是什么
...
一旦 AL 构建了与 AP 调用的服务原语对应的 APDU，就可以使用通用保护机制来应用密码保护。然后产生不受保护或受保护的 APDU ，当长度超过协商的 APDU 长度时，可以采用`通用块传输机制`。
...
GBT 机制支持`双向块传输`、`流传输`和`丢失块恢复`:

- `双向块传输`意味着当一方发送块时，另一方不仅可以确认接收到的块，而且如果它有块可以发送，它也可以发送它们，就是`全双工`模式;
- `流式传输`意味着一方可以发送`多个区块`，而无需另一方对每个区块进行确认,有一个发送窗口;参考 TCP 传输
...
`confirmed AAs`可以在`AA建立期间`协商一致性块，`unconfirmed and pre-established AAs`需要客户端`提前知道`server 的一致性块

##### 9.1.4.8 Maximum PDU size

Client Max Receive PDU Size
: Unsigned16，必须能满足 AARE APDU 大小
: 低于 12 的值被保留，0 表示无限制
...
`pass2后`，如果 application context and xDLMS context`合法`（这两个参数再 pass1 和 2 交换或生成，pass2 后已存在，只不过要到 pass4 全走完才激活），则`授予`当前"Association SN / LN”对象的`reply_to_HLS_authentication`方法`权限`
...
> [信任锚](https://ldapwiki.com/wiki/Trust%20Anchor)见[这篇文章](https://stackoverflow.com/questions/56061101/trust-anchor-certificates-in-pki)，信任锚就是最终信任的那个实体，可以有多个，可以是 root CA，一般操作系统预装了可以信赖的 root CA 列表
{: .prompt-tip }
...
- The general-glo-ciphering and geneal-ded-ciphering xDLMS APDUs
  ...
  The general-ciphering APDU 可以用于客户端和服务器之间，也可以用于第三方和服务器之间。这些`APDU`还携带了所使用`密钥`的`必要信息`。
...
> TODO:`重要！`这里又提到了 Partial service invocations，就是用 FIRST-PART 之类的分包，属于可选的 Additional_Service_Parameters 参数，与 GET 之类的原语的 service-specific block transfer 不一样，和 general block transfer（GBT）又不一样，那这三种分包方式可以同时存在吗 更新：1.Invocation_Type 的分包是针对本地 AL 和本地 AP 之间的，可能是用于 AL 接受缓存或 AP 发送缓存不够的情况。2.service-specific block transfer 是针对本地 AP 和对端 AP 之间的，用于解决超过 AP 接受或发送缓存的情况，这时候还没有 APDU,数据仅仅是原语的参数 3.GBT 是本地 AL 和对端 AL 之间的，这时候分割的 APDU，用于 APDU 过大的情况
{: .prompt-warning }
...
Block_Transfer_Streaming 指示是否允许 AL 使用流（GBT），见 9.4.6.13

Block_Transfer_Window 指示最大接受窗口大小
...
unsolicited 未经请求, unconfirmed 无需确认 或 confirmed 需确认

request 支持分块传输

![m esages](/assets/img/2022-03-17-dlms-green-1/datanotiparams.jpg)

Confirmed：收到 Data-Notification-Confirm APDU
...
> TODO:图上的 EventNotification.reg 单词拼错了，应该是 req
{: .prompt-warning }
...
- client 发起：
  ...
  - 在`unconfirmed`的 AAs 中
    ...
    `unconfirmed` xDLMS services 三种`目的地址`：

    - 单个地址
    - 组地址
    - 广播地址

  如果 AA 没建立，服务端会丢弃 unconfirmed 请求。
...
- 发送接收流程：
  ...
  第一个.response 的 DataBlock_G(见 9.3.7)：

  - Last_Block == FALSE;
  - Block_Number == 1;
  - Result (Raw_Data) == the first K bytes of the encoded data: B1, B2, B3,…., BK.

  客户端 AP 继续发送 GET-REQUEST-NEXT,`Block_Number`和`上一次`接受到的回复`相同`，也就是 1（可以理解为客户端确认序号，表示 1 已确认）。服务端 AP 收到请求后继续发 Block_Number 为 2 的块
...
即使分包，每一包的 Invoke_Id 和 Priority 必须是一样的，因为是同一个请求
...
> TODO:ACCESS 是不支持 service-specific block transfer 吗，文中只给了 GBT 的例子，没有 更新：见 9.4.6.1，一致性块不包含，不支持
{: .prompt-warning }
...
DataNotification.request 原语的`Service_Class`参数：
...
- unconfirmed，支持协议层超时未回应时重发
  ...
  AP 超时未收到回应直接进重发模式，开始重发等待，此时支持协议层返回的信息都忽略，不管成功还是失败，

- confirmed，未收到确认超时时重发

  总是忽视本地确认，未收到远程 AP 确认前总是重发
...
3. general block transfer，由 AP 提供窗口和流参数，但如果没有参数会不会也触发 GBT 有待讨论.AA 握手时 AL 会保存相关参数，见 9.3.2.2 table 54
