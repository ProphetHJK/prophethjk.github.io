...
- COSEM-OPEN;
  ...
  - `unconfirmed AAs`使用COSEM-OPEN服务，可以在单个客户端和多个服务器见建立，只有客户端发送，服务端不回应。
...
在这种情况下，`更多的保护`是指在xDLMS APDU上应用比安全策略所要求的`更多种类`的保护：例如，安全策略要求所有的APDU都经过`认证`，但访问权限要求APDU经过`加密和认证`，即更高的保护。
