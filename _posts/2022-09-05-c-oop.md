...
通过将`基类`属性结构嵌入为`派生类`属性结构的`第一个成员`，可以轻松地在 C 中实现单继承。

![Inheritance](/assets/img/2022-09-05-c-oop/inheritance.jpg)

如图所示，由于super属性是派生类对象中第一个成员，其`起始地址`与派生类对象地址`相同`，通过基类Shape指针即可访问Rectangle对象中的继承部分(super)，这一点在后面实现`多态`时更为重要
