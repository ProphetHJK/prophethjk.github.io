...
通过将`基类`属性结构嵌入为`派生类`属性结构的`第一个成员`，可以轻松地在 C 中实现单继承。这样派生类也包含了基类的所有特性，但较难实现C++语言层级的`选择性继承`和`多继承`等特性。

![Inheritance](/assets/img/2022-09-05-c-oop/inheritance.jpg)

如图所示，由于super属性是派生类对象中第一个成员，其`起始地址`与派生类对象地址`相同`，可以通过调用`->super`方式访问继承部分，也可以通过`基类Shape指针`即可访问Rectangle对象中的继承部分(super)，这一点在后面实现`多态`时更为重要
...
对于基类函数的'this'参数可以使用`(Shape *)&r`或`&r.super`
...
在C语言中也可以为Shape类添加了几个“虚函数”，并由派生类实现
...
虚拟表和多态关系不大，主要是为减少为了实现OOP引入的`函数指针`带来的`空间代价`
...
`虚函数`可以直接定义为`函数指针`，这里是使用了`虚函数表`(ShapeVtbl)，把多个函数指针`压缩`成一个虚表指针，防止每个派生类的对象都包含多个函数指针，其实对于同一个`派生类`，函数指针都指向`同一个`地方，这样会浪费空间

所有C++编译器都通过`每个类`一个`虚拟表` （vtbl） 和`每个对象`一个`虚拟指针` （vptr） 来实现`后绑定`
...
派生类构造函数中应该`覆盖`基类中的虚指针vptr，指向自己的虚表(ShapeVtbl)对象vtbl
