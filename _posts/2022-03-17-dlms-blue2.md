...
- current_credit_status 当前整体状态(所有额度的总体使用情况)，是个 bitmap 每一位表示一类状态
  ...
  - Bit 1 = low_credit, set when the “Account” object’s available_credit level has passed below the “Account” object **low_credit_threshold**。总可用额度大于较低。
    > NOTE 1 The low_credit_theshold attribute of the associated “Credit” object In use is echoed in that attribute.
  - Bit 2 = next_credit_enabled, set when the “Account” object credit_reference_list contains at least one other “Credit” object with non-zero priority, regardless of credit level。除了当前 credit 对象外还有其他 credit 对象(无论其是否耗尽、是否可用)。
  - Bit 3 = next_credit_selectable, set when the next item in the “Account” object credit_reference_list contains a lower priority “Credit” object with non-zero priority with a credit_configuration bit 1 (Requires confirmation) set such that it requires confirmation (for example “Emergency Credit” that is selectable but has not yet been selected)。除了当前 credit 对象外还有其他优先级更低且状态为 `Requires confirmation` 的 credit 对象。
  ...
  - Bit 4 = next_credit_selected, set when the ”Account” object credit_reference_list contains a “Credit” object of lower priority than the current “Credit” object In use, and that lower priority credit object is in the Selected/Invoked state,除了当前 credit 对象外还有其他优先级更低且状态为 `Selected/Invoked` 的 credit 对象。
...
- amount_to_clear 和available_credit类似，是负值的总和(表示需要补缴的 credit 总和)

  - 正常耗尽的 Credit 对象：所有`current_credit_amount负值`（credit_status = (4) Exhausted且credit_configuration bit 2(标志 credit 是否需要偿还)未置位）
  - 需要偿还的 Credit 对象（**replayable** credits）：credit_configuration bit 2置位（credit 需要偿还），其`额度的差值`(current_credit_amount - preset_credit_amount)
  - 保留额度：`clearance_threshold`的负值（value * -1）
...
- credit_reference_list credit对象关联列表，优先级根据 credit 对象的 priority 属性从大到小排序，比如数组第一个元素对应优先级 1，后面是 4、6、11...
...
- next_credit_available_threshold 映射`下一个优先级`credit 对象(credit_reference_list的下一个)的 credit_available_threshold 属性，当没有下一个 credit 对象时，默认为**最大负数**（永远不能切换）
  ...
  > available_credit 仅统计 credit_reference_list 中状态为 Selected/Invoked(2) 或 In use(3) 的

  当 available_credit 小于本值时，说明可用的 credit 即将不足，此时期望能激活下一个 credit 进入 Selected/Invoked(2) 状态等待被使用（不是立即使用，依然优先消耗完当前的 credit），将下一个 credit 对象的 credit_status 置为
  ...
  - Selected/Invoked: 当 credit_configuration 的 bit 1 (Requires confirmation) 未置位，且本值大于 0(说明当前的 credit 还没用完，无需切换，只要置为就绪状态就行)
  - In use: 当 credit_configuration 的 bit 1 (Requires confirmation) 未置位，且本值等于 0(本值为 0，还要求available_credit 小于本值，说明 available_credit 已经用完了，立刻切换到下一个 credit。此时就是**无缝切换**，当前耗尽，立刻切换到下一个 credit。)
