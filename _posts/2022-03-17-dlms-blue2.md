...
#### s4.1.6.1  Date and time formats

DLMS 使用公历(Gregorian calendar)

date 结构和 time 结构都支持通配(wildcards)，当没有通配时其表示明确的日期时间，当使用通配时其可以表示多个日期时间。

可选的值分为三种类型：正常值，通配值，特殊值

二进制为全 1 表示通配(0xFF,0xFFFF)

特殊值：

- month: 0xFD 表示该 date 用于表示夏令时结束，0xFE 表示夏令时开始
- dayOfMonth: 0xFD 表示该月倒数第二天，0xFE 表示该月倒数第一天（如果 dayOfWeek 并非通配，则产生冲突，此时表示当月最后一个周 X，X 为 dayOfWeek。比如 dayOfMonth 为 0xFE，dayOfWeek 为 2，表示当月的最后一个周二）
- 如果 dayOfMonth 和 dayOfWeek 都指定为正常值：
  - 如果年非通配，且月也非通配，也就是日历中的一个明确的月份，此时可以判断 dayOfMonth 和 dayOfWeek 是否匹配，如果不匹配视为不合法。
  - 如果年为通配，此时无法检验 dayOfMonth 和 dayOfWeek 是否匹配，此时规则为等于或大于 dayOfMonth 的第一个 dayOfWeek，比如 dayOfMonth 为 22，dayOfWeek 为 5，表示当月大于等于 22 号的第一个周五(这个例子中，22 是个比较特殊的值，因为每周是 7 天，22 天正好是比完整的 3 周多的那一天，也就是说这个周五必定是当月第四个周五)。
  - 年非通配，月通配的情况未定义

除了正常值、通配和特殊值外的都不合法

date-time 结构是用来表示绝对时间的结构，包含**当地时间**、**与UTC的偏差(分钟)**、**时钟状态**三个信息

其中**时钟状态**如下：

- bit 0 (LSB): invalid value(该时钟非法，比如掉电时间过长，RTC失效，时钟无法恢复)
- bit 1: doubtful value(该时钟非法，后续可以修正，但有效性无法保证)
- bit 2: different clock base(该时钟的时钟源与Clock对象定义的标准时钟源不同)
- bit 3: invalid clock status(本时钟状态失效)
- bit 4: reserved
- bit 5: reserved
- bit 6: reserved
- bit 7 (MSB): daylight saving active(该时间处于夏令时中，比如因为夏令时结束回调时间，同一个时间会出现两次，使用该标志区分是哪个)
...
![alt text](/assets/img/2022-03-17-dlms-blue2/image.png)

如果每个周期独立计算，也就是每个周期从 0 开始，纵坐标为该周期内使用的总电量除以整个周期，所以会随着时间慢慢增加，当周期结束时得到的结果就是该周期内单位时间的平均电量消耗，也就是需量。

![alt text](/assets/img/2022-03-17-dlms-blue2/image-1.png)

如果每次都计算三个周期的电量，当前周期开始时其实是包含前两个周期的的累计电量的，所以不是从 0 开始。但也会出现周期开始时的跳变。
...
- current_average_value `当前平均值`，energy/3t,energy=E2+E3+Δe,t=period,3=周期个数(可以注意到此时实际的统计周期不到3t，应该是 2t 加上 Δe 关联的时间，但还是除以 3t，所以每次周期切换时的平均值会突然比上一平均值小很多)
...
- period `周期`，两次统计的间隔(记录 last_average_value 的间隔)
...
该 IC 允许对不同费率(tariff)策略的处理进行建模。为每个“Register activation”对象分配“Register”, “Extended register”或“Demand register”对象组(register_assignment)，对不同类型的量（例如`有功电量`、`有功需量`、`无功电量`等）进行费率设置。mask_list 定义了不同的`费率策略`（例如，尖、峰、平、谷）。由 active_mask 定义当前激活的费率策略(比如8点到18点激活`平`费率，此时`平`有功电量寄存器和`平`无功电量寄存器激活，计量芯片产生的电量消耗脉冲都附加到这两个寄存器，其他未启用寄存器值不会改变)。默认情况下，未包含在任何“Register activation”对象的 register_assignment 属性中的寄存器始终处于启用状态。
...
> GCP 规定系统必须有 32 个**分费率电量**寄存器，其中 16 个必须是 4 种电能类型 (A+：正向有功电能（Active positive），A-：反向有功电能（Active negative），R+：正向无功电能（Reactive positive），R-：反向无功电能（Reactive negative）) 的 4 个时段费率(如尖、峰、平、谷四个时段) 的电量寄存器，这样就是 4x4=16。其余 16 个可以由厂家根据需求自定义特殊的费率，比如尖峰平谷 4 个段不够，再增加一个 `更低谷` 时间段。
>
> 必须有 10 个**总电量**寄存器，比如正向有功的 4 个费率合并后的总电量寄存器。另外还有总电压，总电流等。
>
> 必须有 20 个**最大需量**寄存器，比如需要保存 4 种类电能类型(`A+`、`A-`、`R+`、`R-`)各 5 个时段(尖、峰、平、谷、自定义)的最大需量
>
> 必须有 4 个**当前需量**寄存器，仅需保存 `A+`、`A-`、`R+`、`R-` 这 4 个
...
- mapping_table 包含status_word到引用表中位置的映射。
  ...
  - ref_table_mapping（choice：long-unsigned或long-unsigned数组）：该值对应`引用表`(reference table，见 6.2.46, 6.2.49, 6.2.50, 6.2.55 and 6.3.7)中的`一条记录`，该记录对应status_word中的第一位，该记录的下一条记录对应status_word中的第二位，直到status_word的最后一位
    ...
    - 如果该值选择`long-unsigned`，则该值表示引用表的起始索引(index)，该值和 status_word 的最高位匹配，以此类推

      假设status_word为10000001，本值为3，表示 status_word 最高 bit 对应引用表的索引 3，次高 bit 对应应用表索引 4
    ...
    - 如果该值选择`array`：array中的每个值直接表示引用表的一个索引(index)，如该值的第一个元素值为 8，表示引用表的索引为 8 的记录对应 status 中的最高位；第二个元素为 10，表示 status 次高位对应引用表索引 10。可以随机对应，而不是像 `long-unsigned` 一样必须顺序对应：
...
- capture_objects 指定分配给“Compact data”对象实例的`COSEM对象属性列表`
  - data_index:是一个指针，它`选择`具有复杂数据类型(结构或数组)的属性的一个或几个`特定元素`，根据捕获数据的类型的不同，有三种情况:
    - 简单类型: 则该值无意义
    - 结构体或数组（非profile generic对象的buffer）: 则该值指向该结构体或数组中的一个或多个特定元素
    - profile generic 对象的 buffer: 该值包含相对选择性访问的参数，如当前日期或记录，有两种选择性访问方式：
      - `相对选择性访问`：相对**当前的**日期或当前的记录返回记录
      - `绝对选择性访问`：根据**指定的**时间范围或记录范围返回记录，由 restriction_element 和 data_index 共同控制，列（columns）是由data_index的MS byte(Most Significant byte,意为最高有效字节)的低半字节（右边4位）决定的
  
    |data_index各字节:|MS-Byte|MS-Byte|LS-Byte|
    |:---:|:---:|:---:|:---:|
    ...
    - 0x0000: 标识整个属性;
    - 0x0001到0x0FFF: 标识非简单类型属性(array/structure)中的一个元素。复杂属性中的第一个元素由data_index 1(0x0001)标识;
    - 0x1000到0xFFFF: 对保存Profile generic对象的buffer的数组进行**相对选择性访问**。通过 data_index 的 MS Byte 高 4 位和 Lower byte 的配合来选择最近时间段内的条目，或最近序号内的条目；通过 data_index 的 MS Byte 低 4 位选择列。
  ...
  - restriction_element: 当被捕获的属性是profile generic对象的属性2(buffer)，且为`绝对选择性访问`方式时，该值生效，用于表示`绝对选择性访问`的参数——时间范围或记录序号范围：
    - 此时的data_index条件：
      - MS Byte高4位为0x0（不为0x0的话表示**相对选择性访问**）
      - MS Byte低4位为0x0到0xF(和**相对选择性访问**的定义相同，表示profile的列选择器)
      - Lower byte为0(不支持和**相对选择性访问**一样的配合MS Byte高4位进行选择的功能)
    - restriction_type表示选择性访问的方式(时间(1)或序号(2))
...
> In ACESM GCP there exists one current AA object representing the information on the currently open AA: Current Association (class_id 15) logical_name: 0-0:40.0.0.255.
>
> 通过 COSEM 接口只能访问到当前本连接对应的 AA 信息，不能获取到其他 AA 信息。比如通过 hdlc 信道使用 client sap 0x01 访问 0x01 服务端，通过该 AA 访问 Current Association 对象，访问到的就是当前 AA 信息。
...
- image_first_not_transferred_block_number 第一个`未传输`的块的编号（最小序号），如果传输全部完成，该编号应该等于或大于image_block_size
...
Push在*push*方法被调用时启动，由`push: Single action schedule对象`、[alarm: Register monitor对象](#6264-alarm-register-alarm-filter-and-alarm-descriptor-objects)、`专用的内部事件`或`外部事件`触发。push操作被`触发`后，根据给定的`push setup`对象中的设置执行。根据`通信窗口`的设置，push会在`随机延迟`后立即执行，或者在通信窗口激活后立即执行。如果push不成功，则会`重试`。重试可能在提示支持协议层失败或缺少确认时发生。
...
- push_object_list 定义要推送的`属性列表`，在调用`push (data)`方法时，内容被发送到`send_destination_and_method`属性中定义的目标。
  ...
  - data_index 参考[compact data](#s4310-compact-data-class_id-62-version--1)
  - restriction,columns 仅当属性是profile generic对象的时，和data_index一起表示选择性访问参数，参考[compact data](#s4310-compact-data-class_id-62-version--1)

  支持三种对 profile 的选择方式，和 compact data 的两种选择方式相比，增加了一种：

  1. 相对范围，**当前时间**之前的一定数量条目
  2. 相对范围，以**最后一条被主站确认的条目**作为起始，其之后的一定数量条目（新增，因为只有push setup有“确认”这个操作）
  3. 绝对范围，由两个边界指定的区间内的条目
  
  data_index 的 MS_Byte upper nibble：
  
  - 0x06...0x02 时，表示方式 2
  - 0x0F...0x07 时，表示方式 1
  - 0 时，表示方式3。此时 restriction 不启用，因为其只用于表示绝对范围。

  > 所以使用该选择性访问方式时，内部需要在主站回复 DataNotification.confim 时将`最后一条被主站确认的条目`信息保存下来。

  data_index MS_Byte lower nibble
...
- send_destination_and_method 包含`目标地址`和`传输服务`(例如电话号码，电子邮件地址，IP地址)，用于发送push_object_list指定的数据
  ...
  - destination 目的地址，取决于transport_service，比如CoAP协议的目的地址是一个CoAP URI,
  - message APDU编码类型
  ...
  然后可以去读取 Push Setup 的 port_reference 属性，获取相应的配置
- communication_window `通信窗口`，包含开始时间和结束时间的区间，是个数组，每个元素为一个区间，只有在区间内的重试才会被发出。如果数组为空，则不限制窗口，随时可以推送
...
- port_reference 通信端口引用，引用"communication port setup"对象。只有 obis ，具体是哪个 class 的，要看 send_destination_and_method 属性，指定的是哪种通信方式
...
- push_operation_method 定义是否使用 `Service_Class` == `Unconfirmed` 或 `Confirmed` 调用 DataNotification.request 服务原语以及推送`重试`的操作`方式`。
  ...
  - (1) unconfirmed, retry on `missing supporting protocol layer confirmation`。AL层无需确认（发送结果不用给AL层），但会在支持层超时未回confirm原语时重发
  - (2) confirmed, retry on `missing confirmation`。必须由AL层确认，否则都会超时重发（即使支持层回了成功也一样）

- confirmation_parameters 指定一个 confirmation 区域，只上报区域内条目。仅当使用*以最后一条被主站确认的条目作为起始进行相对选择性访问的方式*时生效。
  - confirmation_start_date：指定 confirmation 区域起始的时间，不指定表示禁用，也就是全选
  - confirmation_interval(单位:秒)：通过从当前时间(每次push attempt的时间)向前偏移指定 confirmation 区域，比如现在10点，偏移时间为600，表示 9:50-10:00 这个范围。为 0 表示禁用，也就是全选

  ![alt text](/assets/img/2022-03-17-dlms-blue2/image-6.png)

  两个范围的交集就是 confirmation 区域，同时 confirmation 区域与 data_index 选中区域的交集就是需要上报的条目范围。

  主要用途是放弃过旧的未确认条目的上报，尽可能报最新的条目。

  例子：confirmation_start_date 为 0x14190614FF080000FF，表示从 2025年6月20日8点整开始，confirmation_interval 为 18000，表示当前时间向前5小时为范围开始，假设当前为2025年6月20日10点整，向前 5 小时为 2025年6月20日5点整，confirmation_interval 定义的范围为 2025年6月20日5点整 - 2025年6月20日10点整，和 confirmation_start_date 定义的范围的交集为 2025年6月20日8点整 - 2025年6月20日10点整，表示仅允许上报这段时间内的 profile 条目。
...
- protection_parameters_get 包含所有必要的`参数`，用于指定`读取`protection_buffer时应用的`保护`。这个属性首先由`客户端`写入。服务器可能需要填写一些额外的元素，在这种情况下，客户端必须读回这个属性——如果需要的话，转发给第三方——以便他们可以使用这些参数来验证/删除保护。protection_parameters_get 属性是个 array，每个成员都是一层保护，比如加密后数字签名，array 里就会有两个成员
...
- adjust_to_measuring_period (data) 将时间调整为最近(+/-)`测量周期`的起点(需量的测量周期？)
- adjust_to_minute (data) 将时间调整为最近(+/-)整分钟
- adjust_to_preset_time (data) 和preset_adjusting_time配合，激活preset_time，执行时的时间必须是时间范围内的
- preset_adjusting_time (data) 定义一个预设时间preset_time和一个允许执行生效的时间范围
...
- scripts 脚本，操作列表，是一个数组
  - action_specification指向引用对象的动作（限制为`不产生响应`的动作）
    - service_id，表示执行的动作的类型，目前有两种：
      - 写属性
      - 执行方法（仅限无需响应的方法，也就是服务端方法，这类方法都是被客户端调用，而不是主动发起(COSEM 中有些方法是需要返回值的，比如一些 get 方法 get_nbof_mc_IP_addresses)）
...
> 该类设计非常不合理，且没有找到实际应用范例，实际使用中更多会使用 22 类 [single action schedule](#s457-single-action-schedule-class_id--22-version--0)
...
- entries 给定时间执行的脚本
  - schedule_table_entry
    ...
    - validity_window 有效窗口（执行宽容时间），一个以分钟为单位的`时间段`，用于执行时间在**掉电期间**无法执行时，允许的延后执行时间，如果在这个时间内上电，就依然允许执行。(**触发时间switch_time到达后在这个时间范围内必须执行，如果未及时上电导致执行不了，则不执行**)。0xFFFF:无限宽容时间，脚本无论被延后多长时间，都必须被处理;
    - exec_weekdays 定义每周中哪些天有效（比如仅定义周四有效，脚本就只能在周四执行）
    - exec_specdays 见节日表“Special days table”, day_id
    - begin_date和end_date 定义了条目有效的日期范围(允许通配符)。TODO:这里允许通配的意思应该是 begin 和 end 的通配规则是一样的，比如 begin 年通配，end 的年也要通配，表示每年的开始和结束日期。只有一个周表，没有日表，按照这个设计，想定个每月一号的任务也挺麻烦的
...
- Time setting `forward`：向后调整系统时间，调整至更晚的时间，此时操作和掉电相同，根据validity_window补执行
- Time setting `backward`：向前调整系统时间，调整至更早时间，会出现**重复执行**的情况
- Time synchronization：时间校准操作是对时间的一种自动微调（向前或向后调整一个很短的时间），它因保证所有应该执行一次的脚本都仅执行一次，不会出现遗漏或重复执行。和上面两种情况相比更加严格。
- 夏令时向后调整时间则执行所有因此错过的脚本(无视 validity_window)，向前则不执行重复的脚本
...
- entries 数组，每个条目表示一个特殊日期，如圣诞节
  - spec_day_entry
    ...
    - day_id 标识节日（比如有的节日每年的时间不同，又不能用通配符表示，就会出现多条spec_day_entry的day_id相同的情况，每年都有一条），用来和 Schedule 类的 exec_specdays 关联。TODO:实际上是不是还会和 Activity calendar 进行关联，因为 Special days table 并没有绑定 script table，需要与 Schedule 或 Activity calendar 这种可以绑定 script table 的类进行合作。答：是的，Special days table 和 Activity calendar 的 day_profile_table 关联，见 Blue Book Figure 24 – COSEM tariffication model (example):

      ![alt text](/assets/img/2022-03-17-dlms-blue2/image-4.png)
方法：
...
执行周期性的单个定时任务。和 schedule 的区别是一个对象仅对应一个脚本。
...
TODO: actor到底是什么，是客户端吗？答：包括外部的客户端、内部的 schedule 等可以执行 script 对象的都算

属性：

- actions： script 列表，是个数组，一个元素是对`script table`对象的某一个条的引用。比如`[拉闸,合闸,预合闸]`
- permissions_table 每个参与者`权限`，数组每个元素对应每个用户的权限，用户的权限是个位串，其中的位串中每一位对应每个动作权限（与actions对应）。比如`[0b001,0b111]`表示*用户1*有**预合闸**权限，*用户2*有全部权限
- weightings_table 每个参与者`权重`，和permissions_table结构顺序一样，不过把`位串换成了数组`，元素是long-unsigned（最好使用2的幂作为权重值（为了使用 bitmap 管理），而且要保证对于同一个action没有多个用户的权重是相同的，不然两个用户同时触发执行时就不知道该谁执行了(此时按规定大家都不执行)）。比如`[[2,2,4],[1,4,16]]`，表示*用户1*对**拉闸**拥有*权重2*($2^1$)，而*用户2*拥有*权重1*($2^0$)，*用户2*优先执行
- most_recent_requests_table 记录每个用户的最近请求，与 permissions_table 结构完全相同，位串中已执行的置位，不一定只有一个(应该是一次request_action时同时触发两个以上动作)。位的清除在下一次的request_action发生。请求中不包含的动作以及不允许被执行的动作(permissions_table)对应的位会被清除。所以动作正常执行后该位并不会被清除，需要手动清除。
- last_outcome 最近一次执行的动作。用户调用 request_action 后，如果有需要执行的动作要发生，先将动作序号(actions)记录在此处，然后执行动作。（这意味着一次 request_action 只能执行一个 action）

> 可能的 actions：
>
> - disconnect supply(1)
> - enable reconnection(2) 切换到 Ready_for_reconnection
> - reconnect supply(3)
> - prevent disconnection(4) 抑制断开连接（与抑制相关的action的权重估计会比较大）
> - prevent reconnection(5) 抑制重新连接

> permissions_table、weightings_table、most_recent_requests_table 的第一层索引都代表`用户(参与者)`，第二层可能是位图，可能是数组，映射到`动作(actions)`
...
- request_action (data) `封装动作执行请求`，request_actor表示请求用户(上述数组的序号，1表示第一个用户)，request_action_list表示请求动作列表，一次可请求多个，但最终一次请求只能有一个 action(最高权重) 执行。
  ...
  TODO: 仲裁是怎么实现的，如果 request_action 执行时立刻执行动作，那就没有仲裁的概念了。

  > 根据 GCP 定义，request_action 可能同时被 single action scheduler(HES配置)、activity calendar(HES配置) 和 HES(直接) 调用。
...
当`request_action`方法被`actor`调用时，`AP`执行以下活动:
  ...
  5. 如果某个动作拥有一个`唯一`的`最高权重值`，这个值会被写入`last_outcome`属性并执行相应的脚本(TODO:可能并不是立即执行，而是等待所有的竞争者都调用了 request_action后指向)。如果没有最高的`唯一`权重，那么`什么也不会发生`（最高权重不唯一，就全部不执行）。
...
- current_credit_in_use 正在使用的 credit 对象的`引用`
- current_credit_status 当前整体状态(所有额度的总体使用情况)，是个 bitmap 每一位表示一类状态
  - Bit 0 = in_credit, set when the available_credit is above zero。总可用额度大于0。
  - Bit 1 = low_credit, set when the “Account” object’s available_credit level has passed below the “Account” object **low_credit_threshold**。总可用额度较低(低于阈值)。
    > NOTE 1 The low_credit_theshold attribute of the associated “Credit” object In use is echoed in that attribute.
  - Bit 2 = next_credit_enabled, set when the “Account” object credit_reference_list contains at least one other “Credit” object with non-zero priority, regardless of credit level。除了当前 credit 对象外还有其他 credit 对象(除了优先级为 0 的。无论信用级别。)。
  - Bit 3 = next_credit_selectable, set when the next item in the “Account” object credit_reference_list contains a lower priority “Credit” object with non-zero priority with a credit_configuration bit 1 (Requires confirmation) set such that it requires confirmation (for example “Emergency Credit” that is selectable but has not yet been selected)。除了当前 credit 对象外还有其他优先级更低(但非零)且状态为 `Requires confirmation` 的 credit 对象。
    > NOTE 2 The next “Credit” object becomes selectable when the immediately higher priority “Credit” object has reached the “Account” object’s next_credit_available_threshold.
  - Bit 4 = next_credit_selected, set when the ”Account” object credit_reference_list contains a “Credit” object of lower priority than the current “Credit” object In use, and that lower priority credit object is in the Selected/Invoked state,除了当前 credit 对象外还有其他优先级更低的状态为 `In use` 或 `Selected/Invoked` 的 credit 对象。
    > NOTE 3 This is, for example, an “Emergency Credit” which has been selected but is not yet In use.
  - Bit 5 = selectable_credit_in_use, set when the previously selected credit in the “Account” object credit_reference_list is now In use。当前正在使用的 credit 对象是之前被选中的。
  - Bit 6 = out_of_credit, set when the “Credit” object that was In use has been exhausted and no further credit is available without an action on the part of the consumer or other actor。当前 credit 对象已经耗尽，且没有其他 credit 对象可用，等待用户额外的操作。
  - Bit 7 = RESERVED
...
- amount_to_clear 需偿还额度，和 available_credit 类似，是负值的总和(表示需要补缴的 credit 总和)

  - 正常耗尽的 Credit 对象(非 **replayable** credits)：所有`current_credit_amount负值`（credit_status = (4) Exhausted且credit_configuration bit 2 未置位(credit 无需偿还)，非 **replayable** credits，无需 preset_credit_amount）
  - 需要偿还的 Credit 对象（**replayable** credits）：credit_configuration bit 2置位（credit 需要偿还），其`额度的差值`(current_credit_amount - preset_credit_amount)(相当于信用卡有100元预设额度，你用完，当前额度变0，需要还100元才行)
  - 保留额度：`clearance_threshold`的负值(value * -1) (可能是 Account 层面的信用额度)
- clearance_threshold 当额度耗尽后，仅当 available_credit 达到本属性时才能重新启用那些 **replayable** credits(credit_configuration bit 2置位)（TODO: 也许功能类似防抖）
- aggregated_debt `合计欠款`，Charge对象的`total_amount_remaining`的简单合计
- credit_reference_list credit对象关联列表，顺序根据 credit 对象的 priority 属性从小到大排序，比如数组第一个元素对应优先级 1，后面是 4、6、11...
- charge_reference_list charge对象关联列表，顺序同 credit_reference_list
- credit_charge_configuration 这个属性映射出哪些`Charge`将`从哪些Credits`中收取。
  ...
  - Bit 0 允许电源断开时采集
  - Bit 1 允许在负载限制期(load limiting period)采集
  - Bit 2 允许在友好信用期(friendly credit period)内采集(TODO:)
- token_gateway_configuration 此属性用于配置如何`分配`来自“`token_gateway`”的新的`充值token`，以便将token数量的`可配置百分比`分配给每个“Credit”对象。
- account_activation_time 调用activate_account的时间，仅允许特定时间或全通配(表示未定义)，不允许部分通配，未来时间表示将在这个时间执行，过去时间表示已经在这个时间调用过激活。
- account_closure_time 调用close_account的时间，参考 account_activation_time
- currency 本对象的所有方法使用的单位，可以为货币单位、时间、计量单位等(比如余额可以为100元(计费)，可以为100度(计量)，可以为5小时(计时))
- low_credit_threshold `低额度阈值`，映射当前 credit 的 warning_threshold 属性，可以用于低额度时告警。没有其他作用。
- next_credit_available_threshold 映射`下一个优先级`credit 对象(credit_reference_list的下一个)的 credit_available_threshold 属性，当没有下一个 credit 对象时，默认为**最大负数**（永远不能切换）
  
  > 前面提到的 available_credit 属性仅统计 credit_reference_list 中状态为 Selected/Invoked(2) 或 In use(3) 的 credit 对象

  当 available_credit 小于本值时(本阈值由 “next credit” 提供，意思就是由等待被激活的那个 credit 控制什么时候应该激活自己)，说明可用的 credit 即将不足，此时期望能激活下一个 credit 进入 Selected/Invoked(2) 状态，让其准备好被使用（不是立即使用，依然优先消耗完当前的 credit），将下一个 credit 对象的 credit_status 置为

  - Selectable: 当 credit_configuration 的 bit 1 (Requires confirmation) 置位（该 bit 表示 credit 切换到 Selected/Invoked 状态前需要外部操作(按键、执行方法等)，所以这里不能直接切到 Selected/Invoked。对于这类 credit_available_threshold 要配置的大点，留出**充足余量**以便用户操作）
  - Selected/Invoked: 当 credit_configuration 的 bit 1 (Requires confirmation) 未置位，且本值大于 0(说明切换的时机是当前的 credit 还没用完的时机(正常来说是 available_credit 和本值相同时)，无需切换，只要置为就绪状态就行)
  - In use: 当 credit_configuration 的 bit 1 (Requires confirmation) 未置位，且本值等于 0(本值为 0，还要求 available_credit 小于本值，而 available_credit 必须是正值，说明 available_credit 就是 0，已经用完了，立刻切换到下一个 credit。此时就是**无缝切换**，当前耗尽，立刻切换到下一个 credit。)
- max_provision 单位周期内最大收费数量，当Charge为(2) payment_event_based_collection时，规定 charge 在一个周期内能收取的最大费用
- max_provision_period max_provision 的周期
...
![alt text](/assets/img/2022-03-17-dlms-blue2/image-3.png)
...
- limit `透支耗尽阈值`，当`current_credit_amount`小于阈值时 credit_status 变为耗尽`Exhausted`。

  - 在 prepayment 模式下，该值通常设置为零。（通过为 Account 分配一个紧急 credit 对象来避免欠费直接断电）
  - 在 credit 模式下，优先级最高的 credit 对象的 limit 通常等于最大可能的负数（无限透支，但受到 max_provision 限制）。
- credit_configuration 配置本对象行为
  - Bit0 需要`视觉指示`(如液晶屏)
  - Bit1 在credit_status切换到Selected/Invoked之前`需要确认`（如按键）
  - Bit2 `需要偿还`的信用额度(不足 preset_credit_amount 的部分会被计入 Account 的 amount_to_clear，必须全部偿还才能重新启用该 credit)

    示例：预设 preset_credit_amount = 100，使用后 current_credit_amount = -20。已使用金额 = 100 - (-20) = 120，此 120 会加入 amount_to_clear。需充值 120 单位，使 current_credit_amount = 100，才能清除债务。
  - Bit3 `可重置`
  - Bit4 能够从`token`接收信用额度
- credit_status 预付费应用使用，指示当前状态
  - Enabled: `启用`，但还不能使用，会在credit_reference_list中
  - Selectable: `可选择的`，需要其他`命令`才能`激活`，还不能使用。只有 credit_reference_list 中的下一个 credit 才能被设置为该状态。
  - Selected/Invoked：`已选择`，`已激活`，处于排队状态
  - In use：`正在使用中`
  - Exhausted：额度已`耗尽`（只能等待重新激活变为 Enabled 状态）

  ![creditstate](/assets/img/2022-03-17-dlms-blue2/creditstate.jpg)

  > 类比于进程的状态： Selected/Invoked 就是就绪态，随时可调度。In use 就是运行态，只能有一个进程处于该状态。Selectable 就是阻塞态，等待信号(激活命令)才能变成就绪态。Enable 就是新建态。Exhausted 就是终止态。
- preset_credit_amount `初始额度`

  会`累加`到`current_credit_amount`属性的情况：

  - 当 credit_configuration `bit 1`(切换需确认)置位时，用户执行了`确认`操作，credit_status转换为(2)`Selected/Invoked`
  - credit_configuration `bit 1`置位未置位时，credit_status从不为(4)`Exhausted`状态转换为(3)`In use`
...
- update_amount (data) 调整`current_credit_amount`属性的值(增量),`主要是正值`(充值)，负值也允许
...
- charge_type 收费类型
  ...
  - (1) time_based_collection `时间计费`(类似手机卡基本月租费)
...
- token 包含最近接收或标记的未处理的 token， 格式为 octet string，以便在历史 profile 中捕获
...
需要配合两个 Data 类型(Class_id 为 1)的对象表示相关的参数，另见 6.2.17 Payment metering related objects：
...
设备通过监听电话或短信来激活自身的移动网络模块，达到省电目的(不用一直保持 GPRS 网络处于连接状态)

很像老式的传呼机(BP机)，只监听来电和短信，收到通知再去做其他操作，比如找电话亭回电。
...
- number_of_rings TODO:ring是什么意思？环数？响声？答：应该是打电话时的响铃次数
- list_of_allowed_callers 一个可选的主叫号码列表(白名单)，包括呼叫和SMS短信
...
部分事件的产生会触发 Alarm(告警)，此时除了将该事件记录在 Event log profile 之外，还会向主站直接 push。

![alt text](/assets/img/2022-03-17-dlms-blue2/image-2.png)
...
置位表示允许对应的事件触发 Alarm，也就是事件发生会将 Alarm registers 对应位置位

清位表示禁止对应事件触发 Alarm

### Alarm registers(AR)

结构和 Alarm Filters 相同

置位：

- 当事件发生时对应位置位（未被 Alarm Filters 过滤的情况下）

清位：

- 事件结束(比如盖子打开后关上了)时内部可自动清位。
- 客户端可以通过 SET 服务将所有位全清 0（只能全清，参数为 0，不能单个清）

### Alarm descriptors (AD)

结构和 Alarm registers 相同

置位：

- 当 Alarm registers 对应位从 0 变 1 时，对应位变为 1（Alarm registers 清位，或从 1 变为 1 都不会触发本寄存器改变），且本寄存器的位由 0 变 1 时启动上报（原来就是 1，则不启动上报）。

清位：

- 客户端可以通过 SET 服务独立清除每个位（参数取反后和本寄存器进行与操作，比如参数为 0110，表示清中间两位）

> 本寄存器可以理解为主站控制电表允许上报的开关，当上报已经发送时，直到主站清除该位为止，该事件不再被允许上报。

### Alarming process

触发 alarming 时服务端使用 Push Setup 服务将 AD 上报给 HES。

在 ACESM GCP 中，用于监控 AD 的 Register Monitor 属性 2 thresholds 被设置为0。因此，当 AD 值中的任何位从 0 更改为 1 时，将调用“Alarm Monitor”的 action(触发 Push Setup)。

为了确认收到警报，HES 必须通过调用 SET Request 配置 AD 的位。使用 mask 方式，对应位为 1 时情况 AD 的对应位(置零)。

为了重新启用报警报告过程，HES 还必须重置 Alarm Register 中的位，但此时只能全清，不能按位清。
