...
- push_object_list 定义要推送的`属性列表`，在调用`push (data)`方法时，内容被发送到`send_destination_and_method`属性中定义的目标。
  ...
  - restriction,columns 当属性是profile generic对象的时，和data_index一起表示选择性访问参数，参考[compact data](#s4310-compact-data-class_id-62-version--1)

  和 compact data 相比，除了以**当前时间**作为起始进行`相对选择性访问`以及以**指定时间**作为起始进行`绝对选择性访问`外， push setup 的 data_index 增加了以**最后一条被主站确认的条目**作为起始进行`相对选择性访问`的方式，因为只有 push setup 有确认这个概念。当 data_index 的 MS_Byte upper nibble 为 0x06...0x02 时，表示选择从**最后一条被主站确认的条目**开始往后一段时间内的条目。所以使用该选择性访问方式时，内部需要在主站回复 DataNotification.confim 时将`最后一条被主站确认的条目`信息保存下来
...
- confirmation_parameters 指定一个 confirmation 区域，只上报区域内条目。仅当使用*以最后一条被主站确认的条目作为起始进行相对选择性访问的方式*时生效。
  - confirmation_start_date：指定 confirmation 区域起始的时间，不指定表示全选
  - confirmation_interval(单位:秒)：通过从当前时间向前偏移指定 confirmation 区域，比如现在10点，偏移时间为600，表示 9:50-10:00 这个范围。

  ![alt text](/assets/img/2022-03-17-dlms-blue2/image-6.png)

  两个范围的交集就是 confirmation 区域，同时要保证该区域与data_index选中区域的交集就是需要上报的条目范围。
