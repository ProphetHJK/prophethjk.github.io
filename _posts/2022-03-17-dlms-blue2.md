...
该 IC 允许对不同费率(tariff)策略的处理进行建模。为每个“Register activation”对象分配“Register”, “Extended register”或“Demand register”对象组(register_assignment)，对不同类型的量（例如`有功电量`、`有功需量`、`无功电量`等）进行费率设置。mask_list 定义了不同的`费率策略`（例如，`白天`费率策略、`夜间`费率策略）。由 active_mask 定义当前激活的费率策略(比如8点到18点激活白天费率，此时白天有功电量寄存器和白天无功电量寄存器激活，计量芯片产生的电量消耗脉冲都附加到这两个寄存器，其他未启用寄存器值不会改变)。默认情况下，未包含在任何“Register activation”对象的 register_assignment 属性中的寄存器始终处于启用状态。
...
- protection_parameters_get 包含所有必要的`参数`，用于指定`读取`protection_buffer时应用的`保护`。这个属性首先由`客户端`写入。服务器可能需要填写一些额外的元素，在这种情况下，客户端必须读回这个属性——如果需要的话，转发给第三方——以便他们可以使用这些参数来验证/删除保护。TODO: protection_parameters_get是一个数组，推测这个 protection_parameters_get 参数的个数和 protection_object_list 相同，每一项用来加密 protection_object_list 中对应的项
