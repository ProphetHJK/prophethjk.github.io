...
- entries 数组，每个条目表示一个特殊日期，如圣诞节
  - spec_day_entry
    ...
    - day_id 标识节日（比如有的节日每年的时间不同，又不能用通配符表示，就会出现多条spec_day_entry的day_id相同的情况，每年都有一条），用来和 Schedule 类的 exec_specdays 关联。TODO:实际上是不是还会和 Activity calendar 进行关联，因为 Activity calendar 并没有绑定 script table，需要与 Schedule 或 Activity calendar 这种可以绑定 script table 的类进行合作。
