...
>
> 例子：`x = 2;y = (x++, x+1);`，假设逗号`,`不是序列点，则表达式`(x++,x+1)`的执行顺序可以任意，则 y 的值可能为 x+1(x++在之后执行)也就是 3；也可能为 x++后的 x+1,也就是 4，因为没有序列点的情况下`x++`和`x+1`执行顺序可以随意，所以标准规定逗号`,`是一个序列点，则在逗号前，副作用`x++`必须生效，也就是 x++必须先执行，这样 y 的值明确为 4。
>
