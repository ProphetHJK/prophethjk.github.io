...
// 只允许对象为右值情况下使用，返回值的复制。因为为右值的情况下返回后对象会被销毁，其成员不再可用
...
在栈上分配的对象会在函数退出时自动调用其析构函数，利用这个特性，我们可以将指针封装为一个对象，该对象的析构函数就是做 delete 该指针管理的对象的操作，这就是**智能指针**的概念:
...
- 拷贝时执行深拷贝
- 拷贝构造函数不执行拷贝而是**转移所有权**，转移所有权后原指针直接失效，新指针成为唯一管理者。
    但是将拷贝构造函数定义为转移所有权操作不是一件好事情，比如在作为函数参数传递时就会无意识的进行了所有权转移(因为按值传递会进行拷贝构造)，此时实参(智能指针对象)就变成了空指针，无法继续使用，而且当调用的这个函数退出后，这个智能指针对象管理的对象就会被清理。
...
**右值引用**不等于**右值**，以下面代码为例，右值引用 ref 属于**左值**，它的类型是 `int&&` 。**对象的类型**(`int&&`或`int&`)和它的**值类别**(左值或右值)是独立的：
...
下面的例子更能表达**右值引用**不等于**右值**的含义，wrapper 函数内的 x 是左值，所以其可以被修改，当要将其转发给 process 时，必须使用 `std::move(x)` 返回一个右值：
...
void wrapper(int&& x) {
    x=5; // x是左值，所以可以被修改
    // process(x);  // 错误：x为左值，无法直接作为 process 的参数
    process(std::move(x));  // 使用移动语义以右值形式转发给 process
}

// 错误：左值引用不能绑定到右值，wrapper(10)中的10为右值
// void wrapper(int& x) {
//     process(std::move(x));
// }

int main() {
    wrapper(10);  // 输出: Processing rvalue: 5
    return 0;
}
```
...
`std::unique_ptr` 仅实现了移动语义，所以**复制初始化**(copy initialization)和**拷贝赋值**(Copy Assignment)被禁用。当 std::unique_ptr 分配在栈上，当它超出作用域时，它将删除它正在管理的资源：
...
std::unique_ptr 有一个重载的 `operator*` 和 `operator->`，可用于返回正在管理的资源。 `operator*` 返回对托管资源的**引用**，`operator->` 返回**指针**。
...
- 切勿从构造函数或析构函数中调用虚函数：
  ...
  创建派生类时，首先构造基类部分。如果你要从 Base 构造函数调用虚拟函数，并且该类的 Derived 部分尚未创建，则它将无法调用该函数的 Derived 版本。这种情况下，它将调用 Base 版本。
...
虚析构函数是用于实现多态释放，纯虚析构函数是在此基础上增加禁止创建抽象基类的限制。

见[Abstract class](https://en.cppreference.com/w/cpp/language/abstract_class)
...
纯虚只是用来保证子类必须覆盖该函数，纯虚函数自身的**定义**可以提供或不提供，但纯虚析构函数的定义必须提供，因为即使是释放子类对象，编译器保证也一定会调用到基类的析构函数。如果提供纯虚函数的定义，必须放在 class 主体外（语法限制，不能写成`=0`跟上一个函数体）。
...
public:
    // 纯虚析构函数声明
    virtual ~Abstract() = 0;  // 声明为纯虚析构函数
};

// 纯虚析构函数必须有定义，纯虚函数定义要放在class主体外。
Abstract::~Abstract() {
...
int main() {
    Abstract* ptr = new Concrete();  // 父类指针指向子类对象
    // Abstract* ptrb = new Abstract(); // error：纯虚析构函数保证了不能直接定义Abstract对象
    delete ptr;  // 删除子类对象，调用析构函数
    return 0;
}
```
...
Prefer to make interfaces nonvirtual, using Template Method.（public 接口使用非 virtual，其内部调用的具体实现函数使用private + virtual）
...
1. 可以在基类中任意指定 process 的流程，假设我们将 process 拆成了 phase 1 和 phase 2，然后可以在 DoProcessPhase1() 后插入日志打印，再调用 DoProcessPhase2()，又或是在调用 DoProcessPhase2() 前进行前置判断。都只需修改基类的 process() 不用动其他代码。

劣势：

1. 效率降低，对于 IsDone() 来说其实内部只是简单"转发(forward)"了 DoIsDone()，相当于多了一层函数调用。但是其实编译器会进行内联，**不会有任何损耗**。
...
Prefer to make virtual functions private.优先将 virtual 函数置为 private 权限，而不是protected 或 public。

为什么不是 public，见 Guideline #1。为什么不是 protected，见 Guideline #3。

```cpp
class Base {
// foo为private表示子类不可以调用Base::foo()
private:
    virtual int foo_priv(){return 0;}
// foo_prot为protected表示子类可以调用Base::foo_prot()
protected:
    // default implementation
    virtual int foo_prot(){return 1;}
public:
    int fun(){
        return foo_prot();
    }
...
class Derived : public Base {
private:
    int foo_priv() override{
        // error:不允许调用基类的private函数
        Base::foo_priv();
        return 3;
    }
// foo_prot现在为private，表示不希望Derived的子类调用Derived::foo_prot
private:
    int foo_prot() override{
        // 调用基类的protected函数
        return Base::foo_prot();
    }
};
...
int main()
{
    ...
    d.foo_priv(); // error: can't access
    b->foo_priv(); // error: can't access

    return b->fun(); // fun会调用Derived::foo
}
```
...
上例中，除非子类有使用父类 virtual 的需求，才将父类的 virtual 函数设为 protected。比如要在子类的 foo_prot 里调用父类的 foo_prot()(不是自己的 foo)，或是在 foo() 里调用父类的 foo_prot()

#### Guideline #4

A base class destructor should be either public and virtual, or protected and nonvirtual.

析构函数的目的是释放当前类的对象所拥有的资源(清理工作)，为析构函数加上 virtual 后可以通过父类指针执行子类对象的析构函数（最后编译器也会自动调用父类的析构函数）:
...
{
    Base *a = new Derived();
    delete a; // 先调用Derived::~Derived()，再调用Base::~Base()
    return 0; 
}
```

部分规范会要求所有的具有**多态**特性的基类(有 virtual 成员函数)的析构函数必须是 virtual 来保证多态时的析构，但是如果整个项目中没有**通过父类指针析构子类对象**的要求，其实就无需使用该规范。比如如果项目中不使用 new 和 delete，所有对象都是通过退出作用域时的自动释放，就无需使用 virtual 析构：
...
int main()
{
    Derived c;
    Base *a = &c;
    // 并不会通过 a 指针释放 c 对象，析构函数无需 virtual
    return 0; 
}
```

为了保证项目中不会误使用 delete，可以将析构函数置为 protected 权限：
...
int main()
{
    Derived c;
    Base *a = &c;
    delete a; // error: protected的析构函数无法访问
    return 0; 
}
```
...
迭代器(iterator)是指针的**泛化**，它允许 C++ 程序以统一的方式使用不同的数据结构(主要是 stl 提供的容器类，如 vector、array等)

比如对于 vector 来说，内部是一个数组，可以使用 `++pnode` 的方式访问下个元素。而对于 list 结构来说，内部是使用双向链表实现的，每个 node 都有 prev 和 next 指针，对下个元素的访问操作是 `pnode->next`，迭代器相当于封装了这个操作，可以使用 `++it` 的方式直接访问下一个元素，这样就和 vector 及其他容器类统一。

#### 迭代器分类

- LegacyOutputIterator: 支持写入(`*it = 3;`),支持单步移动(每次通过`it`写入后，偏移自动加1)
- LegacyInputIterator: 支持读取(`auto x = *it;`),支持单步移动(每次访问`it`后，偏移自动加1，`auto x1 = *it;auto x2 = *it`)
- LegacyForwardIterator: 在 LegacyInputIterator 基础上，支持多步移动(`++it;`)
- LegacyBidirectionalIterator: 在 LegacyForwardIterator 基础上，支持反向移动(`--it;`)
- LegacyRandomAccessIterator: 在 LegacyBidirectionalIterator 基础上，支持随机访问(`it[n];it=it+n;it=it-n;auto n=it2-it1;`)
- LegacyContiguousIterator: 在 LegacyRandomAccessIterator 基础上，提供完全和指针相同的访问方式。

#### Dereferenceability and validity

Dereferenceability(可解引用性)：迭代器可按可解引用性分为**可解引用**和**不可解引用**两种，用于表示**past-the-end(容器末尾)**的迭代器(如`auto it = v.end();`)具有不可解引用的特性，因为它不指向一个实际存在的元素，属于越界值，只是用来判读是否到达末尾。

validity(有效性): 迭代器可按有效性分为**有效**和**无效**两种，如果迭代器没有被初始化或是其指向的容器发生了变更，这样的迭代器是无效的。注意 `auto it = v.end();` 是有效的，因为它被合法初始化过，只不过未指向实际存在的元素。

对于**无效**的迭代器，C++ 并没有机制去保证禁止对其访问，需要程序员自行处理。

## 参考

- [Learn C++](https://www.learncpp.com/)
- [Abstract class - cppreference.com](https://en.cppreference.com/w/cpp/language/abstract_class)
- [Iterator library - cppreference.com](https://en.cppreference.com/w/cpp/iteratorhttps://en.cppreference.com/w/cpp/iterator)
