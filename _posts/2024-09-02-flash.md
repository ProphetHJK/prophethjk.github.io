...
1. 反向电场：在擦除过程中，通常对源极/漏极/沟道施加较高的正电压，控制栅极施加负电压(或不施加电压)。这形成了一个反向电场。
...
Flash 的存储单元的寿命取决于其隧穿氧化层(浮栅和源漏极间的层)，在擦除和编程的过程中，为了让电子通过量子隧穿效应通过该层，需要在其上施加高压电场，这个过程将会对该层带来不可逆的损耗。对于 NOR Flash 编程时使用的热电子注入方式也会对该层造成损耗。
