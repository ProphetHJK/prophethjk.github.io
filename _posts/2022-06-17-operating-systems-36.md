...
假设块D由字节 d1,…, dn 组成。s1 简单地定义如下：s1 = s1 + di mod 255（在所有 di 上计算）。s2 依次为：s2 = s2 + s1 mod 255（同样在所有 di 上）
...
g(x)和h(x)的`除运算`，可以通过g和h做`xor（异或）`运算。比如将11001与10101做xor运算：

![crc1](/assets/img/2022-06-17-operating-systems-36/crc001.gif)

明白了xor运算法则后，举一个例子使用`CRC-8`算法求101001110100001的效验码。CRC-8标准的h(x) = x^8 + x^7 + x^6 + x^4 + x^2 + 1，既h是9位的二进制串111010101。
...
最基本的方法就是为`每个磁盘扇区（或块）`存储校验和。给定数据块D，我们称该数据的校验和为 C（D）。
...
"`错误位置的写入`（misdirected write）"。这出现在`磁盘和RAID控制器`中，它们正确地将数据写入磁盘，但`位置错误`。在`单磁盘系统`中，这意味着磁盘写入块Dx不是在地址 x（像期望那样），而是在地址 y（因此是“讹误的”Dy）。另外，在`多磁盘系统`中，控制器也可能将 Di，x 不是写入磁盘 i 的 x，而是写入`另一磁盘` j。
