...
位置计数器 `.` 表示当前位置，只能在 SECTIONS 中使用，可以通过赋给一个变量保存下来。比如下面的 `_sdata = .;` 就表示 `_sdata` 保存了 `.data` 输出段的开始位置，后面放置的 `*(.data .data.*);` 了一些输入段，导致 `.` 表示的位置发生移动，接下来 `_edata = .;` 就表示 `.data` 输出段的结束位置。
